...
app.get("/api/robots", (req, res) => {
  try {
    rollbar.info("User got a list of robots.");
    res.status(200).send(bots);
  } catch (error) {
    console.log("ERROR GETTING BOTS", error);
    rollbar.error("Not able to get robots.");
    res.sendStatus(400);
  }
});
...
app.get("/api/robots/five", (req, res) => {
  try {
    ...
    rollbar.info("User got 5 robot choices.")
    res.status(200).send({ choices, compDuo });
  } catch (error) {
    rollbar.error("Could not get five robot choices.")
...
app.get("/api/player", (req, res) => {
  try {
    ...
    rollbar.error("could not get player status")
